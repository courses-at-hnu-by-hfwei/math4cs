...
\begin{frame}{}
  \begin{definition}[$(\U, s) \models \alpha$]
    ...
    \begin{itemize}
      \setlength{\itemsep}{6pt}
      \item 将\red{量词}的论域限制在集合 $|\U|$ 上,
      \item 将$\alpha$中的\teal{常数符号、函数符号、谓词符号}按照\red{\bf 结构} $\U$ 进行\blue{解释},
      \item 将\blue{自由变元} $x$ 解释为 $s(x)$,
      \item 这样就将公式 $\alpha$ 翻译成了某个(数学)领域中的命题,
      \item 然后, 使用(数学)领域知识我们知道该命题成立
    \end{itemize}
  \end{definition}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{}
  \[
    \alpha: \forall x.\; \lnot (x \times x = S{\bf 0} + S{\bf 0})
  \]
